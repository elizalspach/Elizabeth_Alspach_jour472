...
ggplot(aes(x=reorder(school_name, proficient_count), y=proficient_count, weight=proficient_count, fill=proficient_count)) +
  theme_economist() +
  geom_col()+
...
```
#Excel Academy has the highest dropout count besides from the count for the entirety of Baltimore City schools. This means that the academy has the highest number of students who have dropped out in the entire Baltimore City school system in 2020. 145 students is about the same as a high school graduating class in some smaller, rural areas.

Let's make a table with findings of the Baltimore City dropout count:
```{r}
baltcity_dropout %>% 
 ggplot(aes(x=reorder(school_name, dropout_rate_numer), y=dropout_rate_numer, weight=dropout_rate_numer, fill=dropout_rate_numer)) +
  theme_economist() +
  geom_col()+
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))+
    labs(
    title="Schools by dropout count in Baltimore City",
    x = "School Name",
    y = "Dropout Count",
    caption = "source: Baltimore City data. Graph and analysis by Lizzy Alspach")
