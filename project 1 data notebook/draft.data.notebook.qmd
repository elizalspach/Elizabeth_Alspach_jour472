...
janitor:: adorn_totals(baltcity_attendance)
janitor:: adorn_totals(baltcity_enrollment)
```

Now that the data is prepared, it's time to answer some questions. Firstly, what does the education of people in lower median income areas look like in comparison to areas in Baltimore City with higher median incomes?
```{r}
...
```{r}
#Filter x2020_diplomas_certificates for Baltimore City
baltcity_grads <- x2020_diplomas_certificates %>% 
 # slice_max(baltcity_grads)
