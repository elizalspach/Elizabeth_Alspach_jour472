...
**A1** *Montgomery County, Maryland has the largest Hispanic population.*


```{r}
#clean the name for montgomery county
hispanic_clean <- maryland_hispanic %>% 
mutate(hispanic_clean = case_when(name == "Montgomery County,Maryland" ~ "Montgomery County, Maryland", TRUE ~ name))
```

```{r}
#join maryland population and maryland hispanic table by geoid
#could also change the montgomery county name to be cleaned, so there is a space in between "Montgomery County" and "Maryland" in the value. 
updated_hispanic <- hispanic_clean %>% 
  inner_join(maryland_pop, by="geoid")

#find the largest ones
largest_md_hispanic <- updated_hispanic %>% 
  select(geoid, name.x, hispanic_pop, total_pop) %>% 
  mutate(pct_hispanic =(hispanic_pop)/(total_pop)*100) %>% 
  arrange(desc(pct_hispanic))

view(largest_md_hispanic)
```

**Q2**  What is the total population of people who identify as white alone in Maryland? 
**A2**  *There are 3,275,048 people that identify as white in Maryland.*
```{r}
#divided by 100 to get the proper value, since it was unclean to be multiplied by 100
...
**A3** *There is an average 75,669.88 Black population in all Maryland counties.*
```{r}
black_md_clean <- maryland_black %>% 
...
mean(black_md_clean$clean_black_pop)
```
