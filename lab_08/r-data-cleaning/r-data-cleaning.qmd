...
#changed because was overwriting the data frame, so the actual change wasn't showing up when backtracked. renamed to clean
...
A: You would use case_when() to change the titles of the states and counties to match under one title, such as just "Maryland" or "Prince George's." You would have to use this alongside mutate() to actually make a new column and/or adjust the variables within the columns so summarization works properly by renaming values and applying it to all values, to mutate away from undesired variables like "PG" or "Prince George" or "PGC" and such. 
