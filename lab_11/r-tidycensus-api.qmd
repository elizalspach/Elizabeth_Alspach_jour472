...
```{r}
census_api_key("census api key here i removed mine for security", install=TRUE, overwrite = TRUE)
...
```{r}
#California is the biggest state by population
total_pop_2020 <- get_decennial(geography="state", variables="P1_001N", year=2020) %>%
  arrange(desc(value))
...
#California is the state with the highest male population with 19,562,882 men. 
```

```{r}
#now to make it a part of the whole -- join tables
male_percent <- male %>% 
  inner_join(total_pop_2020, by="state") %>% 
  mutate(pct_whole = (estimate/total_population)*100) %>% 
  arrange(desc(pct_whole))

male_percent

#Californian men make up 49.48 percent of the state's total population. The state with the highest male population in proportion to its total population is Alaska, with 52.44 percent and 384,653 males.
```
