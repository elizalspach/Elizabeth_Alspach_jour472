...
```{r}
#The city with the highest absolute change is Beaver Creek CDP with 118,292.
 md_cities %>% 
  mutate(inc_diff = (median_inc_2020 - median_inc_2010)) %>% 
...
```
```{r}
#The city with the highest percentage change is Shaft CDP with a percentage change of 399.88%.
 md_cities %>% 
  mutate(pct_change = (percent(median_inc_2020-median_inc_2010)/median_inc_2010)) %>% 
...
```{r}
#To determine the median value of the percentage change, I would do a median(md_cities$pct_change, na.rm=TRUE) function. THis would filter for the median and pct_change, and na.rm=TRUE would also help to gather all necessary values. 

#Using a separate code block, I would then operate under md_cities, select neighborhood, pct_change and median_inc_2020. Then, I would slice_max for pct_change on one code block, and on a separate one, slice_min for pct_change. On both slice_min and slice_max, I would add n=10. 
```
