...
Count them. 
#97 census tracts are below the citywide median household income for 2020.
```{r}
baltcity_income %>%
  select(Neighborhood, x2020) %>% 
  filter(x2020 < 49875) %>% 
...
#48.5% of the city's census tracts are below the median income in 2020. 
```{r}
below_median_percent <- view((97/200)*100)
```


Draft a tweet with your findings.

#Nearly half of incomes in neighborhoods in Baltimore City fall below the median line, according to an analysis from Baltimore City income data. Read more at [link].
...
baltcity_race %>% 
  ...
  mutate(ratio_w_b = round(x2020_black/x2020_white)) %>% 
  arrange(desc(ratio_w_b)) %>% 
  filter(x2020_white > 0) %>% 
  head()
```
Use r_aggregates_filters from the pre-lab
Use the baltcity_race table, use the mutate command to create a new variable called pct_white which will be the 2020 white population by census tract as a percentage of the entire city's white population for 2020

```{r}
#install Formattable to create percentage in the column
...
baltcity_race %>% 
  ...
  mutate(pct_white = percent(x2020_white/sum(baltcity_race$x2020_white, na.rm = TRUE))) %>% 
  mutate(pct_black = percent(x2020_black/sum(baltcity_race$x2020_black, na.rm = TRUE))) %>% 
  arrange(desc(pct_white)) %>% 
  head()
```
Total white people sum
```{r}
 totalwhite <- sum(baltcity_race$x2020_white, na.rm = TRUE)
#The total white population is 178,996.
```
