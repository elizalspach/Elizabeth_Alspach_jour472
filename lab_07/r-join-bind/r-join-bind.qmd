...
#howard county has the highest census response rate in 2020. 
```
```{r}
#find the county with the lowets census response rate
md_smith_data %>% 
  select(name, census_response_rate2020) %>% 
  slice_min(census_response_rate2020, n=10)
#Worcester County has the lowest census response rate in 2020. 
```
...
# A: I think the tables may not combine the best, since there are different column contents. "inner_join" works best when the columns have the same values, but inside the employer dataframe, the county is named without the state name and comma. This would lead R to confuse the contents, and may actually end up excluding data that may be necessary for analysis. This is similar to the example shown in the textbook, where contents excluding Prince George's County ended up leaving the dataframe with 23 values instead of the many it needed. There will likely be values excluded and information missed. 
